...
layout = dbc.Container([
            ...
            dbc.Row([html.Div(dt.DataTable(data=[{}], id='dt-expression')), html.Div(id='dt-expressionContainer')])])

@app.callback(
    Output('dt-expressionContainer', "children"),
    [Input('btn-executeReport', 'n_clicks')])
def executeCCRCReport(n_clicks):
    if (n_clicks > 0):
        rs2 = ExpressionDao().getExpressionForReport()
        df2 = DataFrame(rs2)
        dt2 = dt.DataTable(
            id='dt-expressionContainer',
                columns=[
